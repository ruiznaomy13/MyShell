/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   redi.c                                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ncastell <ncastell@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/12/07 18:37:49 by ncastell          #+#    #+#             */
/*   Updated: 2023/12/30 13:37:11 by ncastell         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "inc/minishell.h"

void	redi_type(t_all *all, t_process *prcs, int fd_pipe[2], t_process *aux_prcs)
{
	if (prcs->rd->type == RDIN)
		open_infile(all, all->prcs, fd_pipe, aux_prcs);
	else if (prcs->rd->type == RDOUT)
		open_outfile(all, all->prcs, fd_pipe, aux_prcs);
	else if (prcs->rd->type == RDHD)
		here_doc(all->prcs, fd_pipe);
	else if (prcs->rd->type == RDAP)
		apendd(all, all->prcs, fd_pipe,  aux_prcs);
}

void	open_infile(t_all *all, t_process *prcs, int fd_pipe[2], t_process *aux_prcs)
{
	prcs->rd->open = open(prcs->rd->wrd, O_RDONLY);
	if (prcs->rd->open == -1)
	{
		ft_error(all, ACCESS_ERROR, "no such file or directory");
		free_args_and_rd(all, aux_prcs);
		exit(1);
	}
	dup2(prcs->rd->open, STDIN_FILENO);
	close_pipes(fd_pipe);
	close(prcs->rd->open);
}

void	open_outfile(t_all *all, t_process *prcs, int fd_pipe[2], t_process *aux_prcs)
{
	prcs->rd->open = open(prcs->rd->wrd, O_WRONLY | O_TRUNC | O_CREAT, 0666);
	if (prcs->rd->open == -1)
	{
		ft_error(all, ACCESS_ERROR, "no such file or directory");
		free(aux_prcs->rd->wrd);
		free_args_and_rd(all, aux_prcs);
		exit(1);
	}
	dup2(prcs->rd->open, STDOUT_FILENO);
	close_pipes(fd_pipe);
	close(prcs->rd->open);
}

void	apendd(t_all *all, t_process *prcs, int fd_pipe[2], t_process *aux_prcs)
{
	prcs->rd->open = open(prcs->rd->wrd, O_WRONLY | O_APPEND | O_CREAT, 0666);
	if (prcs->rd->open == -1)
	{
		ft_error(all, ACCESS_ERROR, "no such file or directory");
		free_args_and_rd(all, aux_prcs);
		exit(1);
	}
	dup2(prcs->rd->open, STDOUT_FILENO);
	close_pipes(fd_pipe);
	close(prcs->rd->open);
}

void	here_doc(t_process *prcs, int fd_pipe[2])
{
	dup2(prcs->rd->fd_read_hd, STDIN_FILENO);
	close(prcs->rd->fd_read_hd);
	dup2(fd_pipe[0], STDIN_FILENO);
	close(fd_pipe[0]);
}
